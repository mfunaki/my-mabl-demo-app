...
また、/tests/artifacts/ ディレクトリにテスト実行結果のアーティファクト（スクリーンショット、
DOM、HAR、トレース等）が存在する場合は、それらも参照して分析の精度を高めてください。
...
12. **アーティファクト分析**（/tests/artifacts/ が存在する場合）

    アーティファクトが存在するテストについては、以下の追加分析を行ってください：

    **12.1 スクリーンショット分析（screenshots/）**
    - 各ステップの画面遷移が期待通りか
    - UI要素の視認性・配置の問題
    - エラー画面や予期しない状態の検出
    - テストカバレッジの視覚的確認
    ...
    **12.2 DOM分析（doms/）**
    - セレクタの妥当性検証（data-testid、aria属性の存在確認）
    - 動的に生成される要素の特定
    - セレクタ改善の具体的な提案

    **12.3 ネットワーク分析（hars/）**
    - APIレスポンス時間の分析（遅いエンドポイントの特定）
    - エラーレスポンス（4xx, 5xx）の検出
    - 不要なリクエストの特定
    - Wait時間の最適化根拠

    **12.4 トレース・パフォーマンス分析（traces/）**
    - ステップごとの実行時間
    - パフォーマンスボトルネックの特定
    - テスト全体の実行時間の内訳

    **12.5 コンソールログ分析（console_logs/）**
    - JavaScriptエラーの検出
    - 警告メッセージの確認
    - デバッグ情報の漏洩チェック
...
### 8. アーティファクト分析ガイド（/tests/artifacts/ が存在する場合）
