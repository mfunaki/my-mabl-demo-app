...
## 自然言語プロンプト（mabl Test Creation Agent向け）
...
### ステップ 3: 経費の件数・合計金額の確認（生成AIアサーション）
...
- **検証項目（生成AIアサーションで自動判定）**:
  ...
  - 合計金額が表示されていること（例: 「¥3,500」「¥5,000」など）
...
### ステップ 5: 「Taxi」申請を探す（ステータスがPENDINGの経費を探す）
...
### ステップ 7: ステータスの更新確認（ステータスがAPPROVEDに変わることを検証）
...
自然言語プロンプトから自動生成されるテスト手順の対応：

| mablが自動生成するステップ | 対応するテストステップ |
|---------------------------|----------------------|
| ログイン画面で manager / manager123 を入力 | ステップ 1-2: ログインページアクセス・認証 |
| ダッシュボードで経費一覧を表示 | ステップ 4: 経費申請一覧の確認 |
| 経費の件数(未承認件数)、合計金額をチェック | ステップ 3: 件数・合計金額の確認（生成AIアサーション） |
| ステータスが PENDING の経費を探す | ステップ 5: 「Taxi」申請を探す |
| 承認ボタンをクリック | ステップ 6: 承認ボタンをクリック |
| ステータスが APPROVED に変わることを検証 | ステップ 7: ステータスの更新確認 |

**生成AIアサーションの活用ポイント**:
- ステップ 3 の件数・金額チェックでは、具体的な数値を指定せずに「妥当な値か」を自動判定
- 動的に変化するデータに対しても、意味的な検証が可能
